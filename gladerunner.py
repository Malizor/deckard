...
def substitute_templates(ui_string):
    """Substitute templates from a UI string to actual objects

    We can't reliably figure out how these templates are mapped by their true
    consumers, so we use the 'parent' attribute to substitute them.
    This attribute is an annotation used by the Glade program for the same
    purpose, so this seem to be the only right thing to do.
    A valid UI description input is expected (ie. as generated by Glade).
    Return value: the modified ui_string"""
    tree = ET.fromstring(ui_string)
    for template in tree.findall('.//template'):
        if 'parent' not in template.keys():
            # We can't substitute the template (no annotation)
            continue
        template.tag = 'object'
